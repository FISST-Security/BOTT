...
$movie_id = $itemDetails['id'];
 ...
 $sql_pay = "SELECT * FROM  payment where user_id='".$parent_id."' AND movies='movies' AND movie_id='".$movie_id."'";
 ...
 if ($sql_pay_data->num_rows > 0) {
  while($sql_pay_data_row = $sql_pay_data->fetch_assoc()) {
    //print_r($row);
       $movies_array[] = $sql_pay_data_row['movie_id'];
  }
}*/
  /*echo '<pre>';
  pint_r($movies_array);*/
...
if (r == true) {
   window.location.href = 'http://www.vkfashion.co.in/payumoney?user_id=<?php echo $parent_id; ?>&type=<?php echo 'movies'?>&movie_id=<?php echo $movie_id ?>';
} else {
                    ...
                    <div class="uk-grid">
                        ...
                        <?php if(!isWatch()): ?>
                            ...
                            <?php } elseif (($result_get_in->num_rows < $movie_number) || ($update_movie_id == $movie_id))  { ?>
                                <a class="check_view uk-button uk-button-primary uk-button-large uk-width-1-1 uk-margin-top" href="<?php echo gotToWatch();?>"><i class="uk-icon-television uk-margin-small-right"></i> <?php echo _WATCHNOW ?></a>
                           <?php } else { ?>
