<?php 






session_start();

if (isset($_SESSION['user_email'])){

    

    

require '../../config.php';

require '../functions.php';

require '../views/header.view.php';

require '../views/navbar.view.php'; 



$connect = connect($database);

if(!$connect){

	header('Location: ' . _SITE_URL . '/admin/controller/error.php');

	} 



$check_access = check_access($connect);



if ($check_access['user_role'] == 1){



if ($_SERVER['REQUEST_METHOD'] == 'POST'){

	$user_name = cleardata($_POST['user_name']);

	$user_email = cleardata($_POST['user_email']);

	$password = cleardata($_POST['user_password']);

	$user_role = cleardata($_POST['user_role']);

	$encryptPass = hash('sha512', $password);

	$user_pack = cleardata($_POST['user_package']);
	$edgeNode = cleardata($_POST['user_node']);



	$statment = $connect->prepare(

		"INSERT INTO users (user_id,user_name,user_email,user_password,user_role,user_status,package,edgeNode) VALUES (null, :user_name, :user_email, :user_password, :user_role, 1,:user_package,:user_node)"

		);



	$statment->execute(array(

		':user_name' => $user_name,

		':user_email' => $user_email,

		':user_password' => $encryptPass,

		':user_role' => $user_role,
		
		':user_package'=> $user_pack,
		':user_node'=> $edgeNode

		));



	header('Location:' . _SITE_URL . '/admin/controller/users.php');



}



require '../views/new.user.view.php';

require '../views/footer.view.php';



}else{



	header('Location: ' . _SITE_URL);

}



}else {

		header('Location: ./login.php');		

		}





?>