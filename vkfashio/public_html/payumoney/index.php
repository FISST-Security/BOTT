...
if(!empty($series_id)){
$sql = "SELECT * FROM series where serie_id='".$series_id."'";
 $result = $link->query($sql);
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
    //print_r($row);
       $price = $row['serie_price'];
       //$series_price = $row['price'];
  }
}
}
?>
    ...
    <form id="fff" action="<?php echo $action; ?>" method="post" name="payuForm">
      ...
      <table>
        ...
        <tr>
          ...
          <td><input name="amount" type="number" value="<?php echo $price;?>" /></td>
  ...
  <td colspan="3"><textarea name="productinfo"><?php echo $user_id.','.$type.','. $su_id;?></textarea></td>
        </tr>
        <tr>
